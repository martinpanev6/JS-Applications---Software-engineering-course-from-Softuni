function attachEvents() {
    ...
    const loctionInputRef = document.getElementById("location");
    ...
    async function getWeather(ev){
            try {
                const userInput = loctionInputRef.value;
                forecastSectionRef.style.display = "block";
                const locationResponce = await fetch(baseLocationUrl);
    ...
    function generateSpan(classContainer, classSpane, name, data){
        const spanContainer = document.createElement("span");
        spanContainer.classList.add(classContainer);

        const spanName = document.createElement("span");
        spanName.classList.add(classSpane);
        classSpane === "symbol" ? spanName.innerHTML = findSymbol(data.condition) : spanName.textContent = name;

        const degreeSpan = document.createElement("span");
        degreeSpan.classList.add("forecast-data");
        degreeSpan.innerHTML = `${data.low + findSymbol("Degrees")}/${data.high + findSymbol("Degrees")}`;
        ...
        spanContainer.appendChild(degreeSpan);
    ...
    function createForecastTodaySection(data){
        ...
        const degreeSpan = document.createElement("span");
        degreeSpan.classList.add("forecast-data");
        degreeSpan.innerHTML = `${data.forecast.low + findSymbol("Degrees")}/${data.forecast.high + findSymbol("Degrees")}`;
